...
feature "user views list of TV shows" do
  ...
  scenario "view details for a TV show" do
    visit '/television_shows/3'

    expect(page).to have_content("The Newsroom")
    expect(page).to have_content("HBO")
    expect(page).to have_content("2012")
    expect(page).to have_content("2014")
    expect(page).to have_content("Drama")
    expect(page).to have_content("A newsroom undergoes some changes in its workings and " +
      "morals as a new team is brought in, bringing unexpected results for its existing news anchor.")
  end
  ...
  scenario "view details for a TV show with missing information" do
    ...
    expect(page).to have_content("This show is still running.")
  end
end
